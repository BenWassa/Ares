...
- local-only resume via `ares:reading-position:v1`;
...
Do not freeze new child-route slugs before #46 is accepted; route naming is cheap to decide once and expensive to repair after links escape. Preserve existing `/comparison#tempo`, `#full-comparison-depth`, `#full-matrix`, and `#full-analysis` unless a deliberate alias/retirement contract replaces them.
...
`publicationRoutes` currently describes top-level publication structure including glossary/references. #47 will likely need a separate typed guided sequence or equivalent explicit contract so:
...
- safe handling of old `ares:reading-position:v1` state if the state shape/version changes;
