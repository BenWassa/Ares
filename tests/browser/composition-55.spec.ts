...
test('top-level hierarchy remains traversable without JavaScript', async ({ browser }) => {
  ...
  await page.goto('http://127.0.0.1:4321/Ares/'); await page.locator('.home-entry__choices a[href="/Ares/guided"]').click(); await expect(page).toHaveURL(/\/guided$/);
  await page.goBack(); await page.locator('.home-entry__choices a[href="/Ares/cases"]').click(); await expect(page.locator('.case-index a')).toHaveCount(8);
  await page.goBack(); await page.locator('.home-entry__choices a[href="/Ares/full-publication"]').click(); await expect(page.locator('.chapter-directory a')).toHaveCount(8); await context.close();
});
